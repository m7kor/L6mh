    ...
    for (const row of rows) {
        try {
            ...
            if (parsed && typeof parsed === 'object') {
                if (!Array.isArray(parsed.playedIds))
                    parsed.playedIds = [];
                if (!Array.isArray(parsed.failedIds))
                    parsed.failedIds = [];
                if (!Array.isArray(parsed.queue))
                    parsed.queue = [];
                if (typeof parsed.cycleCount !== 'number')
                    parsed.cycleCount = 0;
...
export async function restoreLastVideo(guildId) {
    ...
    if (Array.isArray(saved.playedIds))
        session.playedIds = new Set(saved.playedIds);
    if (Array.isArray(saved.failedIds))
        session.failedIds = new Set(saved.failedIds);
    if (Array.isArray(saved.queue))
        session.queue = saved.queue;
    if (typeof saved.cycleCount === 'number')
        session.cycleCount = saved.cycleCount;
    if (saved.cycleStartedAt)
        session.cycleStartedAt = saved.cycleStartedAt;
...
export function startProgressAutosave(session) {
    ...
    session.progressTimer = setInterval(() => {
        if (!session.current || session.segmentStartedAt == null)
            return;
        const elapsed = Math.max(0, Math.floor(getElapsedSeconds(session)));
...
export function trackRecent(session, videoId) {
    ...
    if (session.recentIds.length > RECENT_HISTORY_SIZE)
...
export async function migrateSessionToQueue(session, catalog) {
    ...
    if (session.playedIds.size === 0 && catalog.length > 0) {
        ...
        if (!session.cycleCount || session.cycleCount < 1)
            session.cycleCount = 1;
        session.cycleStartedAt = new Date().toISOString();
