 ...
 */
import { joinVoiceChannel, createAudioPlayer, createAudioResource, AudioPlayerStatus, VoiceConnectionStatus, entersState, StreamType, NoSubscriberBehavior, } from '@discordjs/voice';
...
import { createAudioStream, preValidateVideo, isLiveStream, getActiveProvider } from '../streaming.js';
import { getSession, saveState, getElapsedSeconds, freezeProgress, startProgressAutosave, stopProgressAutosave, popFromQueue, migrateSessionToQueue, syncQueueWithCatalog, restoreLastVideo, addFailedId, } from '../session.js';
...
async function preloadNextTrack(session) {
    ...
    try {
        ...
        proc.stderr.on('data', () => { });
        proc.on('close', () => { if (session.preloaded?.proc === proc)
...
export async function connectAndPlay(guild, channel, video, { countPlay = true } = {}) {
    ...
    if (session.stallTimeout) {
        clearTimeout(session.stallTimeout);
            ...
            try {
                ...
                if (nextVideoCandidate) {
                    ...
                    if (session.preloaded.proc) {
                        try {
                            session.preloaded.proc.kill('SIGKILL');
                        }
                        catch { }
                    }
                    session.preloaded = null;
                ...
                attempt += 1;
                const delay = jitteredDelay(RETRY_BASE_DELAY_MS, attempt);
                logger.error(`[${guild.id}] Failed to play next (attempt ${attempt}/${MAX_ATTEMPTS}):`, err.message, attempt < MAX_ATTEMPTS ? `retrying in ${delay / 1000}s…` : 'giving up.');
