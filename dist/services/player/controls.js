...
import { getSession, sessions, saveState, getElapsedSeconds, freezeProgress, startProgressAutosave, stopProgressAutosave, } from '../session.js';
...
export async function pausePlayback(guildId) {
    const session = getSession(guildId);
    if (!session.player || !session.current)
        return false;
...
// Queue Management
// ---------------------------------------------------------------------------
export async function removeFromQueue(guildId, index) {
    const session = getSession(guildId);
...
export async function moveToTopQueue(guildId, index) {
    const session = getSession(guildId);
    if (index > 0 && index < session.queue.length) {
...
export function getAllSessions() {
    ...
    for (const [guildId, session] of sessions) {
        if (!session.connection && !session.current)
            continue;
        result.push({
            guildId,
