...
async function alertQuotaExhausted(err) {
    const now = Date.now();
    if (now - quotaAlertSentAt < QUOTA_ALERT_COOLDOWN_MS)
        return;
    quotaAlertSentAt = now;
    await notify('🟡 YouTube API Quota Exhausted', 'The YouTube Data API quota has been used up for the day (resets ~midnight Pacific). '
        + 'Falling back to the last cached video list so playback keeps running — new uploads '
        + "won't show up until the quota resets.\n"
        + `\`\`\`${err.message.slice(0, 400)}\`\`\``, 'warn');
}
...
async function callApi(endpoint, params) {
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            if (res.status === 429) {
                if (attempt === 2)
                    throw new Error(`YouTube API rate limited after 3 retries on ${endpoint}`);
...
async function getUploadsPlaylistId(channelId, apiKey) {
    ...
    if (!uploadsId)
        throw new Error('تعذّر العثور على قائمة فيديوهات هذه القناة.');
 ...
 */
export async function getVideos(channelId = config.channelId, apiKey = config.youtubeApiKey, { useCache = true } = {}) {
    const now = Date.now();
    ...
    try {
        ...
        if (videos.length === 0)
            throw new Error('لا يوجد فيديوهات على هذه القناة.');
...
export async function getVideoDetails(videoId, apiKey = config.youtubeApiKey) {
    ...
    try {
        ...
        const row = db.prepare('SELECT * FROM video_details WHERE video_id = ?').get(videoId);
    ...
    try {
        ...
        VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
      `).run(videoId, item.snippet?.title || null, details.durationSeconds, details.thumbnail, details.viewCount, details.publishedAt);
        }
        catch (dbErr) {
