...
export function killProcesses(session) {
    ...
    if (session.resolveProcess) {
        try {
            session.resolveProcess.kill('SIGKILL');
        }
        catch { }
...
export function createAudioStream(session, youtubeUrl, startSeconds = 0, volume = 100) {
    return new Promise((resolve, reject) => {
        ...
        ytDlpArgs.push('--socket-timeout', '180', '--retries', '15', '--fragment-retries', '30', '--retry-sleep', '5', youtubeUrl);
        const ffmpegArgs = [];
        if (startSeconds > 0) {
            ffmpegArgs.push('-ss', String(startSeconds));
        }
        ffmpegArgs.push('-probesize', '32768', '-analyzeduration', '0', '-i', 'pipe:0', '-bufsize', '512k', '-af', `volume=${volume / 100},afade=t=in:ss=0:d=0.4,aresample=48000`, '-vn', '-f', 's16le', '-ar', '48000', '-ac', '2', 'pipe:1');
        ...
        ytDlpProcess.stderr.on('data', (chunk) => {
            ...
            if (ytDlpStderr.length > STDERR_TAIL_BYTES)
                ytDlpStderr = ytDlpStderr.slice(-STDERR_TAIL_BYTES);
        });
        ...
        ffmpegProcess.stderr.on('data', (chunk) => {
            ...
            if (ffmpegStderr.length > STDERR_TAIL_BYTES)
                ffmpegStderr = ffmpegStderr.slice(-STDERR_TAIL_BYTES);
        });
