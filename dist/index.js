...
import { stopAllSessions, stopPlayback, playRandom, playLatest, resume, playerEvents, getSessionInfo, getAllSessions, getQueue, } from './services/player/index.js';
...
client.once(Events.ClientReady, async (c) => {
    ...
    async function handleDashboardCommand(cmd) {
        ...
        if (lower === 'help')
            return 'Commands: status, np, random, resume, latest, stop, skip, volume <0-100>, play <videoId>';
        ...
        if (lower === '/عشوائي' || lower === 'random') {
            ...
            for (const s of all) {
                ...
                if (guild && guild.members.me.voice.channel) {
                    await playRandom(guild, guild.members.me.voice.channel);
                    done++;
                }
            }
        ...
        if (lower === '/كمل' || lower === 'resume') {
            ...
            for (const s of all) {
                if (s.paused) {
                    try {
                        await resumePlayback(s.guildId);
                        done++;
                    }
                    catch { }
                }
                    ...
                    const guild = c.guilds.cache.get(s.guildId);
                    if (guild && guild.members.me?.voice?.channel) {
                        await resume(guild, guild.members.me.voice.channel);
                        done++;
                    }
                }
            }
        ...
        if (lower === '/اخر_مقطع' || lower === 'latest') {
            ...
            for (const s of all) {
                const guild = c.guilds.cache.get(s.guildId);
                if (guild && guild.members.me.voice.channel) {
                    await playLatest(guild, guild.members.me.voice.channel);
                    done++;
                }
            }
        ...
        if (lower === '/ايقاف' || lower === 'stop') {
            ...
            for (const s of all) {
                try {
                    await stopPlayback(s.guildId, { manual: true });
                    done++;
                }
                catch { }
            }
        ...
        if (lower.startsWith('volume ')) {
            ...
            if (isNaN(val) || val < 0 || val > 100)
                return 'Invalid volume. Use 0-100.';
            const vol = val / 100;
            const { setVolume } = await import('./services/player/index.js');
            let done = 0;
            for (const s of all) {
                try {
                    setVolume(s.guildId, vol);
                    done++;
                }
                catch { }
            }
        ...
        if (lower.startsWith('qrm ')) {
            ...
            let done = 0;
            for (const s of all) {
                if (await removeFromQueue(s.guildId, index))
                    done++;
            }
        ...
        if (lower.startsWith('qtop ')) {
            ...
            let done = 0;
            for (const s of all) {
                if (await moveToTopQueue(s.guildId, index))
                    done++;
            }
        ...
        if (lower.startsWith('play ')) {
            ...
            if (!video)
                return 'Video not found in catalog.';
            let done = 0;
            for (const s of all) {
                const guild = c.guilds.cache.get(s.guildId);
                if (guild && guild.members.me.voice.channel) {
...
process.on('unhandledRejection', (reason) => {
    ...
    if (reason?.message?.includes('Cannot perform IP discovery'))
        return;
    if (reason?.code === 'EAI_AGAIN' || reason?.code === 'ENOTFOUND' || reason?.code === 'ETIMEDOUT')
        return;
...
function gracefulShutdown(signal) {
    ...
    for (const [, timeout] of voiceActionTimeouts)
